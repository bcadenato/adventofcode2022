...
size_used = calc_size(tree)

size_missing = SIZE_NEEDED - (SIZE_TOTAL - size_used)

print(f'Used size is {size_used}\nMissing size is {size_missing}')

tree_dirs_sorted = {k: v for k, v in sorted(tree_dirs.items(), key = lambda item: item[1])}

for dir, size in tree_dirs_sorted.items():
    if size < size_missing:
        continue
    print(f'The folder to delete is {dir} with size {size}')
